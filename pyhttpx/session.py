...
class CookieJar(object):
    ...
    def __init__(
        self, name=None, value=None, expires=None, max_age=None, path=None, domain=None
    ):
        self.name = name
        self.value = value
...
class HTTPSConnectionPool:
    ...
    def _new_conn(self):
        ...
        conn = context.wrap_socket(sock=None, server_hostname=None)
...
class HttpSession(object):
    ...
    def request(
        ...
        if _cookies:
            send_kw["cookie"] = "; ".join(
                "{}={}".format(k, v) for k, v in _cookies.items()
            )
    ...
    def prep_request(self, req, send_kw) -> bytes:
        msg = b"%s %s HTTP/1.1\r\n" % (
            req.method.encode("latin1"),
            req.path.encode("latin1"),
        )
    ...
    def http2_send(self, req):
        ...
        self.stream_id = 1
        self.settings = bytes.fromhex(
            "505249202a20485454502f322e300d0a0d0a534d0d0a0d0a00001e0400000000000001000100000002000000000003000003e800040060000000060004000000000408000000000000ef0001"
        )
        ...
        if _cookies:
            for k, v in _cookies.items():
                head_block.append(("cookie", f"{k}={v}"))
