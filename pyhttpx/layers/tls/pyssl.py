...
class SSLContext:
    ...
    def set_payload(
        ...
        self.supported_groups = b"".join(
            [struct.pack("!H", i) for i in self.supported_groups]
        )
...
class TLSSocket:
    ...
    def connect(self, addres, timeout=None, proxies=None, proxy_auth=None):
        ...
        if self.proxies and self.proxies.get("https"):
            ...
            self.sock.set_proxy(
                SocketProxy.HTTP, proxy_ip, proxy_port, username, password
            )
        ...
        while True:
            ...
            if content_type == 0x16:
                if not self.server_change_cipher_spec:
                    ...
                    self.tls13 = (
                        True
                        if self.servercontext.serverstore.ext.get(43) == b"\x03\x04"
                        else False
                    )
                    ...
                    if self.servercontext.serverstore.ext.get(16):
                        ...
                        self.context.application_layer_protocol_negotitaion = (
                            alpn.decode("latin1")
                        )
                    ...
                    while plaintext:
                        ...
                        if handshake_type == 0x14:
                            ...
                            if self.tls_cxt.certificate_request:
                                ...
                                ciphertext = self.tls_cxt.encrypt(
                                    certificate_data + b"\x16", b"\x17"
                                )
                            ...
                            self.tls_cxt.server_ctx.crypto_alg.key = (
                                self.tls_cxt.server_application_write_key
                            )
                            self.tls_cxt.server_ctx.crypto_alg.fixed_iv = (
                                self.tls_cxt.server_application_write_iv
                            )

                            self.tls_cxt.client_ctx.crypto_alg.key = (
                                self.tls_cxt.client_application_write_key
                            )
                            self.tls_cxt.client_ctx.crypto_alg.fixed_iv = (
                                self.tls_cxt.client_application_write_iv
                            )
                            return
                            ...
                            while payload:
                                ...
                                if ext_type == b"\x00\x10":
                                    self.context.application_layer_protocol_negotitaion = data[
                                        3:
                                    ].decode(
                                        "latin1"
                                    )
            ...
            if not self.tls13:
                if self.servercontext.done and exchanage:
                    self.tls_cxt.server_ctx.random = (
                        self.servercontext.serverstore.random
                    )
                    self.tls_cxt.negotiated.ciphersuite = int(
                        self.servercontext.serverstore.cipher_suit.hex(), 16
                    )
                    self.tls_cxt.rsa_pulicKey = (
                        self.servercontext.certificatecontext.rsa_pulicKey
                    )
                    ...
                    if self.tls_cxt.certificate_request:
                        ...
                        certificate = b"\x16\x03\x03\x00\x07" + bytes.fromhex(
                            "0b000003000000"
                        )
                        self.tls_cxt.handshake_data.append(
                            bytes.fromhex("0b000003000000")
                        )
                        self.sock.sendall(certificate)
                    keychange = ClientKeyExchange(self.tls_cxt.publickey_bytes).dump(
                        self.tls_cxt
                    )
                    changecipherspec = ClientCpiherSpec().dump()
    ...
    def sendall(self, plaintext):
        ...
        while plaintext:
            ...
            write_buff = (
                b"\x17" + b"\x03\x03" + struct.pack("!H", len(ciphertext)) + ciphertext
            )
