...
class TLSSessionCtx(object):
    def __init__(self, client=True):
        ...
        name_curve = 0x001D
        self.group_x25519_key = (
            CryptoContextFactory.crypto_container[name_curve]
            .client_kx_privkey.public_key()
            .public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)
        )

        self.client_secp_kx_privkey = CryptoContextFactory.crypto_container[
            0x0017
        ].client_kx_privkey
    ...
    def encrypt(self, P, content_type):
        ...
        if self.cls_cipher_alg.type == "block":
            tbd = (
                struct.pack("!Q", self.client_ctx.sequence)
                + content_type
                + self.tls_version
                + struct.pack("!H", len(P))
                + P
    ...
    def decrypt(self, C, content_type):
        ...
        if self.tls13:
            ...
            p_len = (
                len(C)
                - self.cls_cipher_alg.nonce_explicit_len
                - self.cls_cipher_alg.tag_len
            )
    ...
    def load_alg(self):
        ...
        kx_alg, cipher_alg, hmac_alg, hash_alg, tls1_3 = get_algs_from_ciphersuite_name(
            cipher_name
        )
    ...
    def load_key(self):
        ...
        if cipher_type == "aead":
            ...
            self.client_fixed_iv = self.key_block[
                key_len * 2 : key_len * 2 + fixed_iv_len
            ]
            self.server_fixed_iv = self.key_block[
                key_len * 2 + fixed_iv_len : key_len * 2 + fixed_iv_len * 2
            ]
            ...
            self.client_write_key = self.key_block[
                mac_key_len * 2 : mac_key_len * 2 + key_len
            ]
            self.server_write_key = self.key_block[
                mac_key_len * 2 + key_len : mac_key_len * 2 + key_len * 2
            ]

        self.client_ctx.crypto_alg = self.cls_cipher_alg(
            self.client_write_key, self.client_fixed_iv
        )
        self.server_ctx.crypto_alg = self.cls_cipher_alg(
            self.server_write_key, self.server_fixed_iv
        )
    ...
    def negotiated_premaster_secret(self):
        if self.kx_alg.startswith("ECDHE"):
            ...
            if name_curve == 0x001D:
                ...
                client_kx_privkey = CryptoContextFactory.crypto_container[
                    name_curve
                ].client_kx_privkey
                ...
                self.premaster_secret = client_kx_privkey.exchange(
                    cg_ec.ECDH(), server_publickey
                )
    ...
    def verify_server_message(self, server_verify_data):
        ...
        handshake = self.hash_alg(b"".join(self.handshake_data)).digest()
        tmp = prf(self.master_secret, label + handshake, self.hash_alg, outlen=12)
    ...
    def compute_verify_data(self):
        verify_data = self.hkdf.compute_verify_data(
            self.secrets["client_handshake_traffic_secret"],
            b"".join(self.handshake_data),
        )

        # verify_data_len=3byte
        verify_data = (
            b"\x14" + struct.pack("!I", len(verify_data))[1:] + verify_data + b"\x16"
        )
        return verify_data
    ...
    def make_secret(self, server_publickey):
        ...
        client_handshake_traffic_secret = self.secrets[
            "client_handshake_traffic_secret"
        ]
        server_handshake_traffic_secret = self.secrets[
        ...
        if sslkey_file_name:
            with open(sslkey_file_name, "a") as f:
                s = (
                    f"CLIENT_HANDSHAKE_TRAFFIC_SECRET {self.client_ctx.random.hex()} {client_handshake_traffic_secret.hex()}\n"
        ...
        self.client_ctx.crypto_alg = self.cls_cipher_alg(
            self.client_handshake_write_key, self.client_handshake_write_iv
        )
        self.server_ctx.crypto_alg = self.cls_cipher_alg(
            self.server_handshake_write_key, self.server_handshake_write_iv
        )

    def derive_application_traffic_secret(self):
        client_application_traffic_secret = self.hkdf.derive_secret(
            self.tls13_master_secret, b"c ap traffic", b"".join(self.handshake_data)
        )
        server_application_traffic_secret = self.hkdf.derive_secret(
            self.tls13_master_secret, b"s ap traffic", b"".join(self.handshake_data)
        )

        self.client_application_write_key = self.hkdf.expand_label(
            client_application_traffic_secret, b"key", b"", self.hkdf.key_len
        )
        self.client_application_write_iv = self.hkdf.expand_label(
            client_application_traffic_secret, b"iv", b"", 12
        )

        self.server_application_write_key = self.hkdf.expand_label(
            server_application_traffic_secret, b"key", b"", self.hkdf.key_len
        )
        self.server_application_write_iv = self.hkdf.expand_label(
            server_application_traffic_secret, b"iv", b"", 12
        )
