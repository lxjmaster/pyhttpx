...
class CertificateContext:
    ...
    def load(self, flowtext, serverstore):
        ...
        if serverstore.cipher_name.startswith("TLS_RSA"):
            self.publickey_bytes = OpenSSL.crypto.dump_publickey(
                OpenSSL.crypto.FILETYPE_PEM, self.cert.get_pubkey()
            )
            self.rsa_pulicKey = rsa.PublicKey.load_pkcs1_openssl_pem(
...
class ServerContext:
    ...
    def load(self, flowtext):
        ...
        if handshake_type == 0x02:
            ...
            self.certificatecontext = CertificateContext().load(
                flowtext, self.serverstore
            )
        elif handshake_type == 0x0C:
            self.curve_name = flowtext[5:7]
            if not self.curve_name in [
                b"\x00\x1d",
                b"\x00\x17",
                b"\x00\x18",
                b"\x00\x19",
...
class KeyStore:
    ...
    def init(self):
        ...
        if cipher_type == "aead":
            ...
            self.server_mac_key = self.keyBlock[
                cipher_suit["mac_key_len"] : cipher_suit["mac_key_len"] * 2
            ]
            self.client_write_key = self.keyBlock[
                cipher_suit["mac_key_len"] * 2 : cipher_suit["mac_key_len"] * 2
                + key_len
            ]
            self.server_write_key = self.keyBlock[
                cipher_suit["mac_key_len"] * 2
                + key_len : cipher_suit["mac_key_len"] * 2
                + key_len * 2
            ]
