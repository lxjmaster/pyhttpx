...
class SocketProxy(socket.socket):
    ...
    def set_proxy(
        self, proxy_type, proxy_addr, proxy_port, username=None, password=None
    ):
        self.proxy = (proxy_type, proxy_addr, int(proxy_port), username, password)

    def connect(self, dest_pair):
        (proxy_type, proxy_addr, proxy_port, username, password) = self.proxy
        ...
        if username and password:
            http_headers.append(
                ...
                + b64encode(
                    username.encode("latin1") + b":" + password.encode("latin1")
                )
            )
        ...
        try:
            ...
            raise ProxyError("Proxy server connection time out")
