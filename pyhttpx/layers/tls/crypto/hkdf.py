...
class TLS13_HKDF(object):
    ...
    def make_secret(self, tls13_dhe_secret, handshake_messages):
        ...
        client_handshake_traffic_secret = self.derive_secret(
            tls13_handshake_secret, b"c hs traffic", b"".join(handshake_messages)
        )

        server_handshake_traffic_secret = self.derive_secret(
            tls13_handshake_secret, b"s hs traffic", b"".join(handshake_messages)
        )

        # traffic
        client_handshake_write_key = self.expand_label(
            client_handshake_traffic_secret, b"key", b"", self.key_len
        )
        client_handshake_write_iv = self.expand_label(
            client_handshake_traffic_secret, b"iv", b"", 12
        )
        server_handshake_write_key = self.expand_label(
            server_handshake_traffic_secret, b"key", b"", self.key_len
        )
        server_handshake_write_iv = self.expand_label(
            server_handshake_traffic_secret, b"iv", b"", 12
        )
        ...
        client_application_traffic_secret = self.derive_secret(
            tls13_master_secret, b"c ap traffic", b"".join(handshake_messages)
        )
        server_application_traffic_secret = self.derive_secret(
            tls13_master_secret, b"s ap traffic", b"".join(handshake_messages)
        )
        client_application_write_key = self.expand_label(
            client_application_traffic_secret, b"key", b"", self.key_len
        )
        client_application_write_iv = self.expand_label(
            client_application_traffic_secret, b"iv", b"", 12
        )

        server_application_write_key = self.expand_label(
            server_application_traffic_secret, b"key", b"", self.key_len
        )
        server_application_write_iv = self.expand_label(
