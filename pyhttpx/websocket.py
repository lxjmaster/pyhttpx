...
class WebSocketClient:
    def __init__(
        self, url=None, headers=None, loop=None, ja3=None, exts_payload=None, ping=False
    ):
    ...
    async def close(self):
        await self.send(
            struct.pack("!H", 1000).decode("latin1"), binary=True, opc=0b1000
        )
        self.open = False
    ...
    async def connect(self):
        ...
        context.set_payload(
            browser_type="chrome", ja3=self.ja3, exts_payload=self.exts_payload
        )
    ...
    def check_proto(self, data):
        ...
        if status_code == "101":
            ...
            sec_websocket_key = (
                self.sec_websocket_key + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11"
            )
            sec_websocket_accept = head["sec-websocket-accept"]

            b = base64.b64encode(
                hashlib.sha1(sec_websocket_key.encode("latin1")).digest()
            )
            if b != sec_websocket_accept.encode():
                raise SecWebSocketKeyError("sec_websocket_key verify failed")
        else:
            raise SwitchingProtocolError(
                f"host={self.addres[0]},path={self.path},switching protocol error,status_code {status_code},text: {data}"
            )
